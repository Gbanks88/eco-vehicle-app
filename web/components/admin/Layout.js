...
export default function AdminLayout({ children, session }) {
  ...
  // Use either server-side session or client-side session
                  ...
                  <nav className="mt-5 space-y-1 px-2">
                    {navigation.map((item) => (
                      <Link
                        key={item.name}
                        href={item.href}
                        className="group flex items-center px-2 py-2 text-base font-medium rounded-md text-white hover:bg-green-700"
                      >
                        <item.icon
                    ...
                    <div className="flex items-center">
                      <div>
                        <img
                          ...
                          src={`https://ui-avatars.com/api/?name=${currentSession?.user?.name || 'Admin'}&background=random`}
                          alt=""
                        />
                      </div>
                      <div className="ml-3">
                        <p className="text-base font-medium text-white">{currentSession?.user?.name || 'Admin'}</p>
                        <button
                          onClick={() => signOut()}
            ...
            <nav className="mt-5 flex-1 space-y-1 px-2">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="group flex items-center px-2 py-2 text-sm font-medium rounded-md text-white hover:bg-green-700"
                >
                  <item.icon
          ...
          <div className="flex flex-shrink-0 border-t border-green-700 p-4">
            <div className="group block w-full flex-shrink-0">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-white">{currentSession?.user?.name || 'Admin'}</p>
                  <button
                    onClick={() => signOut()}
